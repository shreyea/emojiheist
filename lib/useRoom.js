...
export function useRoom(roomId, playerName) {
  ...
  // Keep a ref of latest state for the sync handler (avoids stale closures)
  ...
  // Host interval for round/planner timer (resilient to host handoff)
  useEffect(() => {
    if (!isHost || !roundStartTime || !channelRef.current) return;
    if (phase !== "planner" && phase !== "guessing") return;

    let hintSent = hintRevealed; // assume already sent if true
    ...
    const interval = setInterval(() => {
      ...
      if (phase === "guessing" && remaining <= hintAt && !hintSent) {
        hintSent = true;
      ...
      if (remaining <= 0) {
        ...
        // Auto-submit if planner time is up
        if (phase === "planner") {
          channelRef.current.send({
            type: "broadcast",
            event: "emoji_plan",
            payload: { emojiPlan: stateRef.current?.emojiPlan || "" },
          });
        }
      }
    }, 1000);
    ...
    return () => clearInterval(interval);
  }, [isHost, roundStartTime, phase, settings.roundTime, hintTime, hintRevealed]);

  // Host interval for vote timer (resilient to host handoff)
  useEffect(() => {
    if (!isHost || !voteStartTime || phase !== "voting" || !channelRef.current) return;
  ...
  useEffect(() => {
    ...
    channel
      // --- Presence: reliable player tracking ---
      .on("presence", { event: "sync" }, () => {
        ...
        for (const key of Object.keys(presenceState)) {
          ...
          if (latest.role === "spectator") {
            newSpectators.push({ id: key, name: latest.name });
          } else {
            newPlayers.push({
              id: key,
              name: latest.name,
              score: scoresRef.current[key] ?? 0,
              joinedAt: latest.joinedAt || 0,
            });
          }
        }

        // Sort by join time so first player is always host
      ...
      .on("broadcast", { event: "sync_state" }, ({ payload }) => {
        if (payload.players) {
          const scoreMap = {};
          payload.players.forEach((p) => { scoreMap[p.id] = p.score || 0; });
          scoresRef.current = { ...scoresRef.current, ...scoreMap };
      ...
      .on("broadcast", { event: "request_sync" }, () => {
        ...
        if (s.players && s.players.length > 0 && s.players[0].id === playerId.current) {
          channel.send({
            type: "broadcast",
            event: "sync_state",
            payload: s,
      ...
      .subscribe(async (status) => {
        if (status === "SUBSCRIBED") {
          ...
          // Track presence — this reliably notifies all subscribers
          await channel.track({
            name: playerName,
            role: "player",
          ...
          // Request game state sync if reconnecting mid-game
  ...
  const submitGuess = useCallback(
    (text) => {
      if (!channelRef.current || !mission) return;
      
      // Check similarity locally before broadcasting
      const { match } = checkSimilarity(text, mission.sentence);
  ...
  const startVoting = useCallback(() => {
    ...
    // Interval logic moved to useEffect
  }, [isHost]);
